...
const zipName = "UPLOAD-TO-GHOST-bomsociety-theme-v1.3.5.zip";
...
async function createInspectableTheme({ homeMarker = true } = {}) {
  ...
  await writeFile(join(theme, "package.json"), '{"version":"1.3.5"}');
...
test("version 1.3.5 controls the exact upload ZIP filename", () => {
  assert.equal(JSON.parse(themePackage).version, "1.3.5");
  assert.match(build, /UPLOAD-TO-GHOST-bomsociety-theme-v\$\{themePackage\.version\}\.zip/);
  assert.match(workflow, /UPLOAD-TO-GHOST-bomsociety-theme-v1\.3\.5\.zip/);
});
test("existing homepage-capable templates contain the stable nonvisual marker", () => {
  assert.match(footer, /Theme 1\.3\.5 · Commit/);
...
test("the built ZIP contains markers in every required homepage template", () => {
  ...
  const zip = "releases/UPLOAD-TO-GHOST-bomsociety-theme-v1.3.5.zip";
...
test("upload and activation use the inspected v1.3.5 ZIP and returned Ghost name", () => {
