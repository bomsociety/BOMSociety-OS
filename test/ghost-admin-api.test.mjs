...
const zipName = "UPLOAD-TO-GHOST-bomsociety-theme-v2.0.1.zip";
...
async function createInspectableTheme({ homeMarker = true } = {}) {
  ...
  await writeFile(join(theme, "package.json"), '{"version":"2.0.1"}');
...
test("version 2.0.1 controls the exact upload ZIP filename", () => {
  assert.equal(JSON.parse(themePackage).version, "2.0.1");
...
test("existing homepage-capable templates contain the stable nonvisual marker", () => {
  assert.match(footer, /Theme 2\.0\.1 · Commit/);
...
test("the built ZIP contains markers in every required homepage template", () => {
  ...
  const zip = "releases/UPLOAD-TO-GHOST-bomsociety-theme-v2.0.1.zip";
...
test("upload and activation use the inspected v2.0.1 ZIP and returned Ghost name", () => {
